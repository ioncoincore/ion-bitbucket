...
class MempoolCoinbaseTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        spends1_id = [ self.nodes[0].sendrawtransaction(tx, True, False, True) for tx in spends1_raw ]

        blocks = []
        blocks.extend(self.nodes[0].generate(1))

        spends2_raw = [ create_tx(self.nodes[0], txid, node0_address, 499.99) for txid in spends1_id ]
        spends2_id = [ self.nodes[0].sendrawtransaction(tx, True, False, True) for tx in spends2_raw ]
