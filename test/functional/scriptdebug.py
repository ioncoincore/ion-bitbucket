...
class DebugSession:
    ...
    def __init__(self, prevTx, spendTx, flags=None, prevoutIdx=None):
        ...
        self.flags = flags
        if self.flags is None: self.flags = ionlib.ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS

        self.spendScript = self.spendTx.vin[self.inputIdx].scriptSig
        self.redeemScript = None

        self.sm = ionlib.ScriptMachine(flags=self.flags, tx=self.spendTx, inputIdx=self.inputIdx, inputAmount=self.inputAmount)
    ...
    def evalWithReturn(self, script):
        ...
        if result[0] != ionlib.ScriptError.SCRIPT_ERR_OK:
        ...
        if len(stk): # outside of your script, the VM pops 1 thing off the stack and uses that to decide whether the script is successful
            ...
            if self.flags & ionlib.ScriptFlags.SCRIPT_VERIFY_CLEANSTACK:
                if len(stk) > 1:
                    print("Script finished with result %s but unclean stack" % hexlify(stkTop))
                    return (ionlib.ScriptError.SCRIPT_ERR_CLEANSTACK, result[1])
            if ord(stkTop) == 1:
                return (ionlib.ScriptError.SCRIPT_ERR_OK, result[1])
            else:
                return (ionlib.ScriptError.SCRIPT_EVAL_FALSE, result[1])
        return None
    ...
    def evals(self, script):
        ...
        try:
            ...
            if str(e) == 'stepped beyond end of script':
                return (ionlib.ScriptError.SCRIPT_ERR_OK, pos)
            else:
...
class ScriptDebugTest (BitcoinTestFramework):
    ...
    def runScriptMachineTests(self):
        ...
        # flags = ionlib.ScriptMachine.SCRIPT_VERIFY_P2SH | ionlib.ScriptMachine.SCRIPT_VERIFY_STRICTENC | ionlib.ScriptMachine.SCRIPT_VERIFY_LOW_S | ionlib.ScriptMachine.SCRIPT_VERIFY_NULLFAIL | ionlib.ScriptMachine.SCRIPT_VERIFY_CHECKSEQUENCEVERIFY | ionlib.ScriptMachine.SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY | self.SCRIPT_VERIFY_DERSIG

        flags = ionlib.ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS

        print("eval scriptSig")
        sm = ionlib.ScriptMachine(flags=flags, tx=tx, inputIdx=0, inputAmount=1*ionlib.BCH)
    ...
    def runDSVtest(self):
        ...
        dbg = DebugSession(prevTx, spendTx, flags=ionlib.ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS )
        print("Evaluating spend script")
        dbg.evalSpendScript()
        print("Evaluating constraint script")
        result = dbg.evalConstraintScript()
        assert(result[0] == ionlib.ScriptError.SCRIPT_ERR_NUMBER_BAD_ENCODING)

        dbg = DebugSession(prevTx, spendTx, flags= (ionlib.ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS) & ~ionlib.ScriptFlags.SCRIPT_VERIFY_MINIMALDATA)
        print("Evaluating spend script")
        dbg.evalSpendScript()
        print("Evaluating constraint script")
        result = dbg.evalConstraintScript()
        assert(result[0] == ionlib.ScriptError.SCRIPT_ERR_OK)
